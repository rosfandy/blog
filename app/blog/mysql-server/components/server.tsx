...
export default async function ServerComponent() {
    const post = await fetchPageBySlug("mysql-server");
    if (!post) notFound();
    ...
    //     fs.writeFileSync('./public/mysql-thumbnail.jpg', imageData);
    // } catch (error) {
    //     console.log(error)
    // }
    ...
    fs.writeFileSync('./public/mysql.json', JSON.stringify(jsonContent, null, 2));
}